...
## 2026-07-18 22:03 — 首次容器构建 SSH 会话中断

- 现象：生产构建读取基础镜像元数据时，腾讯云主动关闭 SSH 连接；现有健康容器继续运行。
- 根因：一次短暂 SSH 会话中断，服务器负载正常且没有遗留构建进程。
- 处理：改用服务器后台构建并轮询日志，app/web 镜像从缓存完成构建后再滚动重建容器。

## 2026-07-18 22:05 — 新模板仍命中一周不可变 CSS 缓存
...
## 2026-07-18 22:43 — 并行部署覆盖生产路由与演示密码配置

- **现象**：产品视觉库 GET 请求稳定返回 405，登录页密码占位回退为“请输入密码”。
- **根因**：另一项共创流程在服务器工作树中用旧版 `routes/web.php` 与 `config/geoflow.php` 覆盖当前版本，误删产品 GET 路由和 `login_demo_password_enabled` 配置；重建容器后旧文件进入生产镜像。
- **处理**：仅在对方未提交修改上补回两处既有定义，保留全部共创路由与后台路径改造；更新容器文件、重建路由/配置缓存并重启 PHP-FPM，产品页恢复且演示密码校验重新启用。
## 2026-07-18 22:26 — 共创系统首次部署被上传路径校验拦截
...
## 2026-07-18 23:30 — 知识库首页手机端文字被压成竖排

- **根因**：页头始终使用桌面 `flex` 单行布局，四个长按钮与标题共同争抢 390px 宽度；列表操作区另有 440px 内联固定宽度。
- **解决方案**：页头在 `lg` 以下纵向堆叠，操作入口改为响应式网格，并让列表操作区手机端占满可用宽度。
- **复现**：390px 视口下标题和按钮逐字换行，页面主内容宽度远超可读范围。

## 2026-07-18 23:33 — 空知识库夹具未渲染列表操作区

- **根因**：响应式回归测试从 HTTP 响应断言列表操作区，但测试数据库没有知识库记录，Blade 的非空分支不会渲染。
- **解决方案**：页头继续使用响应断言，条件分支内的固定宽度改为直接读取模板源码断言。
- **复现**：`test_knowledge_base_index_links_to_the_catalog` 找不到仅存在于非空列表分支的 `lg:w-[440px]`。

## 2026-07-19 22:06 — 本地响应式验收受网络与 SQLite 基线阻断

- **现象**：MacBook Air 直连腾讯云生产 IP 超时；备用本地 SQLite 启动又在既有敏感词迁移的临时表语法处失败。
- **根因**：当前网络需经本机代理访问腾讯云，且该历史迁移不兼容 SQLite；均与首页产品视觉库入口改动无关。
- **解决方案**：通过 HTTP CONNECT 建立只读 SSH 转发，在真实生产容器中完成登录、样式和跳转验收；临时转发层修正绝对 URL 端口后即清理。
